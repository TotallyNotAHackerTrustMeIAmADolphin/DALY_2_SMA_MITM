...
// of logTo's own format specifiers.
static void test_percent_inside_an_argument_passes_through(void)
{
    logTo(capture, "%s", "battery at 50% and falling");
...
// A truncated line still ends in '\n' - otherwise the next line written
// would run onto it (Serial and the SD .log are both plain byte streams).
static void test_truncated_line_still_ends_in_newline(void)
{
    char big[400];
    memset(big, 'x', sizeof(big) - 1);
    big[sizeof(big) - 1] = '\0';
...
int main(int, char **)
{
    ...
    RUN_TEST(test_truncated_line_still_ends_in_newline);
    return UNITY_END();
}
